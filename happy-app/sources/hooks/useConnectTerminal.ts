...
export function useConnectTerminal(options?: UseConnectTerminalOptions) {
    ...
    const processAuthUrl = React.useCallback(async (url: string) => {
        // SELF-HOSTED: Accept either full happy://terminal?KEY URL or just the KEY directly
        // This works around Android QR scanner URL validation issues
        let keyPart: string;
        if (url.startsWith('happy://terminal?')) {
            keyPart = url.slice('happy://terminal?'.length);
        } else if (url.match(/^[A-Za-z0-9_-]{20,}$/)) {
            // Looks like a base64url key (alphanumeric, underscore, dash, 20+ chars)
            keyPart = url;
        } else {
